...
import numpy as np
from graph_optimizer.utils.tf.graph_utils import load_graph
from graph_optimizer.utils.tf.generators import create_complex_concat_graph
    ...
    for node in graph_def.node:
        if node.op == "Placeholder":
            ...
            if tensor.shape.ndims == 0:
                ...
                shape = tuple(
                    max(1, d) if d is not None and d >= 0 else 1
    ...
    try:
        ...
        for test_idx in range(num_tests):
            ...
            if test_idx == 0:
                ...
                for name in data_names[:5]:
                    ...
                    if isinstance(data, np.ndarray):
                        ...
                        print(
                            f"      values={data.flatten()[:8]}{'...' if data.size > 8 else ''}"
                        )
                    else:
                        print(f"    {name}: scalar={data}")
                print(
                    f"    ... ({len(data_names)} data placeholders, {len(random_data) - len(data_names)} frozen vars)"
                )
            ...
            for i, name in enumerate(output_nodes):
                ...
                print(
                    f"      Original:  {orig.flatten()[:10]}{'...' if orig.size > 10 else ''}"
                )
                print(
                    f"      Optimized: {opt.flatten()[:10]}{'...' if opt.size > 10 else ''}"
                )
...
def main():
    ...
    print(
        f"Reduction: {len(original_graph.node) - len(optimized_graph.node)} nodes "
    ...
    all_passed = run_consistency_tests(
        original_graph, optimized_graph, output_nodes, num_tests=20
    )
