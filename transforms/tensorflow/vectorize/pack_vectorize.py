...
Pack Vectorize Pass
===================
Hoists a Pack node above its inputs' common operation, batching the operation
instead of running it once per branch.
...
Supported ops: unary elementwise (Relu, Sigmoid, …), binary elementwise
(Add, Mul, …), MatMul → BatchMatMulV2, StridedSlice (elimination path),
and dimension ops (Reshape, Squeeze, ExpandDims, Transpose).

Registration: name="pack_vectorize", opt_level=3, priority=50.
"""
    ...
    def _execute_hoist(self, optimizer, pack_node, analysis: HoistAnalysis) -> Dict:
        ...
        for idx, info in analysis.inputs.items():
            ...
            if info.action == "broadcast":
                ...
                input_rank = (
                    len(analysis.main_input_shape)
                    if analysis.main_input_shape is not None
                    else 3
                )
                ...
                if config.op_type == "Transpose":
                    ...
                    new_val = self._adjust_axis(
                        old_val, analysis.output_batch_axis, input_rank
                    )
            ...
            for name, val in first_op.attr.items():
                if name not in config.remove_attrs and name not in {
                    ...
                    if config.needs_axis_adjust and name == config.axis_attr_name:
                        ...
                        new_axis = self._adjust_axis(
                            old_axis, analysis.output_batch_axis, input_rank
                        )
