...
class ConcatCombinePass(PatternRewritePass):
    ...
    def _combine_concat(self, match, optimizer):
        ...
        for input_name in root_values_inputs:
            ...
            if base_name in optimizer.nodes:
                ...
                if input_node.op == "ConcatV2":
                    logging.debug(
                        f"[ConcatCombine] Found inner ConcatV2: {input_node.name}"
                    )

                    # Analyze inner inputs
                    inner_all_inputs = input_node.input
                    inner_control_inputs = [
                        i for i in inner_all_inputs if i.startswith("^")
                    ]
                    inner_data_inputs = [
                        i for i in inner_all_inputs if not i.startswith("^")
                    ]
        ...
        final_inputs = (
            new_values_inputs + [root_axis_input] + sorted(list(new_control_inputs))
        )

        new_node = create_node(
            "ConcatV2", root.name, inputs=final_inputs, attr=root.attr
        )
        new_node.attr["N"].i = len(new_values_inputs)

        logging.info(
            f"[ConcatCombine] Combined: {root.name} ({len(root_values_inputs)} -> {len(new_values_inputs)} value inputs)"
        )
        return [new_node]
