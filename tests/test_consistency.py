...
from graph_optimizer.utils import (
    create_node,
...
class TestPassConsistency(unittest.TestCase):
    ...
    def _run_and_compare(
        self, original_graph_def, optimized_graph_def, output_nodes, feed_dict_base=None
    ):
        ...
        for orig_name in output_nodes:
            ...
            if any(n.name == orig_name for n in opt_def.node):
                ...
                candidates = [
                    n
                    for n in opt_def.node
                    if n.name.startswith(orig_name + "/") or orig_name in n.name
                ]
                ...
                if not candidates:
                    ...
                    leaf_nodes = [
                        n
                        for n in opt_def.node
                        if n.name not in consumer_map
                        and n.op not in ["Placeholder", "Const"]
                    ]
                    if leaf_nodes:
                        ...
                        raise ValueError(
                            f"Cannot find output node {orig_name} or its renamed version in optimized graph. Available nodes: {[n.name for n in opt_def.node]}"
                        )
                ...
                if leaf_candidates:
                    ...
                    selected = max(
                        leaf_candidates, key=lambda n: get_numeric_suffix(n.name)
                    )
                    ...
                    orig_node = next(
                        (n for n in orig_def.node if n.name == orig_name), None
                    )
                    if orig_node:
                        same_op_candidates = [
                            n for n in candidates if n.op == orig_node.op
                        ]
        ...
        output_tensors_orig = [
            g_orig.get_tensor_by_name(n + ":0") for n in output_nodes
        ]
        output_tensors_opt = [
            g_opt.get_tensor_by_name(n + ":0") for n in optimized_output_nodes
        ]
        ...
        for i, (o, p) in enumerate(zip(res_orig, res_opt)):
            self.assertTrue(
                np.allclose(o, p, atol=1e-5),
                f"Output mismatch at {output_nodes[i]} (optimized: {optimized_output_nodes[i]})",
            )
    ...
    def test_pack_vectorize_complex(self):
        ...
        self.assertTrue(
            len(m_batch_nodes) >= 1,
            "Should have at least one BatchMatMul node after hoisting",
        )
    ...
    def test_algebraic_simplify_complex(self):
        ...
        graph_def.node.extend(
            [x, y, z, zero, one, add0, mul1, sub_y, div_z, final_add1, final_add2]
        )
        ...
        pipeline = OptimizationPipeline(
            graph_def=graph_def, level=3, output_nodes=["final_add2"]
        )
        report = pipeline.run()
    ...
    def test_constant_fold_complex(self):
        ...
        pipeline = OptimizationPipeline(
            graph_def=graph_def, level=3, output_nodes=["out"]
        )
        report = pipeline.run()
        optimized = report.graph_def
    ...
    def test_multi_pass_integration(self):
        ...
        graph_def.node.extend(
            [x1, x2, zero, one, a1, m1, c2, c3, add_cf, a2, pack, out]
        )
